...
function onHoverUpdate(x, y)
	RFIAModifierStack.hoverDisplay(getCounterAt(x, y));
end

function onHover(oncontrol)
	if not oncontrol then
		RFIAModifierStack.hoverDisplay(0);
	end
end
...
function onClickRelease(button, x, y)
	...
	if n ~= 0 then
		RFIAModifierStack.removeSlot(n);
	end
	return true;
end
...
function onDrop(x, y, draginfo)
	...
	if sDragType == "number" then
		...
		RFIAModifierStack.addSlot(sDragText, draginfo.getNumberData());
		return true;
	...
	elseif acceptdrop and acceptdrop[1] and acceptdrop[1].type then
		...
		for _,v in pairs(acceptdrop[1].type) do
			if v == sDragType then
				draginfo.setSlot(1);
				RFIAModifierStack.addSlot(draginfo.getStringData(), draginfo.getNumberData());
				return true;
			end
		end
	end
